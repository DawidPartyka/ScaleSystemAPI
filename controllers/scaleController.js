...
exports.getById = async (req, res) => {
    ...
    const scale = await scaleHelper.findById(
        req.user.id, scaleId,
        await Management.count({ where: { UserId: req.user.id } })
    );
    const status = scale.result ? scale.value.length === 0 ? 404 : 200 : 500;

    res.status(status)
        .send(scale.result ? scale.value : { msg: 'Sorry, something went wrong on server side' });
}
